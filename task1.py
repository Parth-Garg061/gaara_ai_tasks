...
def check_pose(landmarks, w, h, phase):
    feedback = []
    correct = True
    ...
    if l_ankle_lm.visibility > 0.5 and r_ankle_lm.visibility > 0.5:
    ...
    if phase == "REST":
        ...
        if not arms_down:
            feedback.append("Lower your arms to rest")
            correct = False
        return correct, feedback, True

    head_size = abs(nose[1] - l_ear[1])
    hands_up = (l_wrist[1] < nose[1] - head_size) and \
               (r_wrist[1] < nose[1] - head_size)
    if not hands_up:
    ...
    arms_near_ears = (left_arm_to_ear < shoulder_width * 0.4) and \
                     (right_arm_to_ear < shoulder_width * 0.4)
...
class BreathCounter:
    def __init__(self):
        ...
        self.inhale_detected = False
        self.THRESHOLD = 6

    def update(self, landmarks, w, h):
        l_shoulder = get_coords(landmarks, "LEFT_SHOULDER", w, h)
        r_shoulder = get_coords(landmarks, "RIGHT_SHOULDER", w, h)
        avg_y = (l_shoulder[1] + r_shoulder[1]) / 2

        if self.prev_shoulder_y is not None:
            movement = self.prev_shoulder_y - avg_y

            if movement > self.THRESHOLD:
                self.inhale_detected = True

            if movement < -self.THRESHOLD and self.inhale_detected:
                self.count += 1
                self.inhale_detected = False

        self.prev_shoulder_y = avg_y
...
def main():
    ...
    with mp_pose.Pose(
        ...
        while cap.isOpened():
            ...
            target_breaths = HOLD_BREATHS if phase == "ARMS_UP" else REST_BREATHS
            ...
            if exercise_done:
                ...
                breath_counter.update(landmarks, w, h)
                breath_count = min(breath_counter.count, target_breaths)

                correct, feedback, arms_down = check_pose(landmarks, w, h, phase)

                if correct and breath_count >= target_breaths:
                    if phase == "ARMS_UP":
                        phase = "REST"
                        breath_counter.reset()
                    else:
                ...
                dot_style  = mp_drawing.DrawingSpec(color=skeleton_color, thickness=4, circle_radius=5)
                line_style = mp_drawing.DrawingSpec(color=skeleton_color, thickness=3)
